...
app.get('/api/events/export', requireEnterprise, async (req, res) => {
  ...
  try {
    ...
    // Query Event table (scoped to this user if found)
    const eventWhere = {
    ...
    // Also pull SystemLog entries (motion webhook events, no user scoping)
    const logsWhere = {
    ...
    const logs = await db.systemLog.findMany({
      ...
      take: Math.min(limit, 2000), // cap logs separately
    ...
    // SystemLog rows (same columns where possible, blank for event-only fields)
    for (const l of logs) {
      const row = [
